...
TEST_CASE("Testing the readDataFromCSV function") {
    ...
    CHECK(lines == std::vector<std::string> {"HEADER", "month, money spent", "jan, 150", "feb, 200"});
    ...
    CHECK(classified_info.getDatas() == std::vector<Data> {Data{"jan", 150}, Data{"feb", 200}});
    CHECK(classified_info.getDatasMap() == std::map<std::string, double> {{"jan", 150}, {"feb", 200}});
}
